...
const el={
 ...
 marketView:$("#market-view"),portfolioView:$("#portfolio-view"),communityView:$("#community-view"),
 pValue:$("#portfolio-value"),pUnreal:$("#portfolio-unrealised"),pReal:$("#portfolio-realised"),pTotal:$("#portfolio-total"),
 pChart:$("#portfolio-chart"),pTip:$("#portfolio-tooltip"),holdings:$("#holdings-list"),trades:$("#trade-list"),
 dialog:$("#stock-dialog"),close:$("#dialog-close"),dSector:$("#detail-sector"),dName:$("#detail-name"),dTicker:$("#detail-ticker"),
 dDescription:$("#detail-description"),dPrice:$("#detail-price"),dOwned:$("#detail-owned"),dAverage:$("#detail-average"),dRange:$("#detail-range"),
 dChart:$("#detail-chart"),dTip:$("#detail-tooltip"),shares:$("#trade-shares"),estimate:$("#trade-estimate"),fee:$("#fee-note"),
 submit:$("#trade-submit"),tradeMessage:$("#trade-message"),max:$("#trade-max"),communityHoldings:$("#community-holdings"),feedNote:$("#real-market-feed-note")
};
const sectors={technology:"Technology",semiconductors:"Semiconductors and AI",internet_media:"Internet and entertainment",consumer:"Consumer and retail",finance:"Finance and payments",industrial:"Industry and aerospace",energy:"Energy",healthcare:"Healthcare",high_volatility:"High volatility"};
let overview=null,selectedTicker=null,detailDuration="1h",tradeSide="buy",portfolioDuration=localStorage.getItem("real-stock-portfolio-duration")||"30d";
let detailChartType=localStorage.getItem("real-stock-detail-chart-type")==="candlestick"?"candlestick":"line";
...
function createCompany(c){
 ...
 const copy=document.createElement("div");copy.innerHTML=`<span class="ticker">${c.ticker}</span><h2>${c.name}</h2><small class="volatility-badge ${c.volatility_tier}">${String(c.volatility_tier||"medium").toUpperCase()} VOLATILITY</small>`;
 ...
 watch.onclick=async e=>{e.stopPropagation();try{const {error}=await supabaseClient.rpc("toggle_real_stock_watchlist",{p_ticker:c.ticker});if(error)throw error;await loadOverview(true)}catch(err){message(err.message,"error")}};
 head.append(copy,watch);
 const pr=document.createElement("div");pr.className="card-price";
 const hasPrice=Number(c.price)>0;
 pr.innerHTML=`<div class="price-block"><strong>${hasPrice?price(c.price):"Waiting…"}</strong><span>${hasPrice?"chips per share · tracks USD quote":"first Finnhub quote pending"}</span></div><span class="change ${cls(c.change_24h)}">${hasPrice?percent(c.change_24h):"—"}</span>`;
 const chart=svg("svg",{class:"spark","aria-hidden":"true"});miniChart(chart,c.sparkline,Number(c.change_24h)<0);
 const foot=document.createElement("div");foot.className="card-foot";foot.innerHTML=`<span>${sectors[c.sector]||c.sector}</span><strong>${Number(c.owned_shares)>0?`${shares(c.owned_shares)} shares · ${number(c.position_value)} chips`:"No position"}</strong>`;
...
function renderHeader(){
 ...
 el.regime.textContent=m.market_open?"Open":"Closed · cached trading";el.regime.className=m.market_open?"positive":"neutral";
 el.lastTick.textContent=overview.last_refresh_at?`${time(overview.last_refresh_at)} · ${ago(overview.last_refresh_at)}`:"No quotes cached yet";nextTick=new Date(overview.next_refresh_at).getTime();
 if(m.last_error){el.feedNote.textContent=`Finnhub cache warning: ${m.failed_symbols} symbol${Number(m.failed_symbols)===1?"":"s"} failed on the latest refresh. Previous valid prices remain available.`;el.feedNote.classList.add("warning")}else{el.feedNote.textContent=m.market_open?`Finnhub cache healthy · ${number(m.updated_symbols)} companies refreshed. This is simulated trading, not real investing.`:"The US market is closed. Cached prices remain visible and simulated trading stays available.";el.feedNote.classList.remove("warning")}
}
...
async function loadCommunity(force=false){
 ...
 try{const {data,error}=await supabaseClient.rpc("get_public_real_stock_holdings");if(error)throw error;communityData=data;renderCommunity()}
...
function renderAll(){renderHeader();renderGrid();renderMetrics();renderHoldings();renderTrades();setView(localStorage.getItem("real-stock-view")||"market");updateEstimate()}
async function loadOverview(silent=false){if(busy)return;busy=true;try{const {data,error}=await supabaseClient.rpc("get_real_stock_market_overview");if(error)throw error;overview=data;communityData=null;renderAll();if(selectedTicker&&el.dialog.open)updateDetail(company(selectedTicker))}catch(e){console.error(e);if(!silent)message(e.message||"The real market could not be loaded.","error")}finally{busy=false}}

function setView(view){
 if(!["market","portfolio","community"].includes(view))view="market";localStorage.setItem("real-stock-view",view);
 $$(".view-tab").forEach(b=>{const a=b.dataset.view===view;b.classList.toggle("active",a);b.setAttribute("aria-pressed",a)});
 el.marketView.classList.toggle("hidden",view!=="market");el.portfolioView.classList.toggle("hidden",view!=="portfolio");el.communityView.classList.toggle("hidden",view!=="community");
...
async function loadPortfolio(d){
 portfolioDuration=d;localStorage.setItem("real-stock-portfolio-duration",d);$$("#portfolio-ranges button").forEach(b=>b.classList.toggle("active",b.dataset.duration===d));
 try{const {data,error}=await supabaseClient.rpc("get_my_real_stock_portfolio_history",{p_duration:d});if(error)throw error;portfolioData=data;lineChart(el.pChart,el.pTip,data.points,"total",d,v=>number(v),"chips")}catch(e){console.warn(e)}
}
function updateDetail(c){if(!c)return;el.dSector.textContent=(sectors[c.sector]||c.sector).toUpperCase();el.dName.textContent=c.name;el.dTicker.textContent=c.ticker;el.dDescription.textContent=c.description;el.dPrice.textContent=Number(c.price)>0?`${price(c.price)} chips`:"Waiting for quote";el.dOwned.textContent=`${shares(c.owned_shares)} shares`;el.dAverage.textContent=`${price(c.average_cost)} chips`;el.dRange.textContent=Number(c.day_low)>0&&Number(c.day_high)>0?`${price(c.day_low)} – ${price(c.day_high)}`:"—";updateEstimate()}
async function loadDetail(){const {data,error}=await supabaseClient.rpc("get_real_stock_company_history",{p_ticker:selectedTicker,p_duration:detailDuration});if(error)throw error;detailData=data;drawDetailChart()}
...
function updateEstimate(){const c=company(selectedTicker);if(!c||!(Number(c.price)>0)){el.estimate.textContent="Waiting for quote";el.fee.textContent="Trading unlocks after the first Finnhub refresh";return}const sh=shareValue(),ep=Number(c.price)*(tradeSide==="buy"?1.0025:.9975),gross=tradeSide==="buy"?Math.ceil(ep*sh):Math.floor(ep*sh),feeRate=tradeSide==="buy"?.0015:.0025,fee=gross?(tradeSide==="buy"?Math.max(0,Math.round(gross*feeRate)):Math.max(1,Math.ceil(gross*feeRate))):0,total=tradeSide==="buy"?gross+fee:Math.max(gross-fee,0);el.estimate.textContent=tradeSide==="buy"?`${number(total)} chips spent`:`${number(total)} chips received`;el.fee.textContent=`Estimated at ${price(ep)} per share · ${number(fee)} fee (${(feeRate*100).toFixed(2)}%)`}
function maxShares(){const c=company(selectedTicker);if(!c||!(Number(c.price)>0))return;if(tradeSide==="sell")el.shares.value=Number(c.owned_shares).toFixed(4);else{const wallet=Number(overview.portfolio.wallet_chips),v=Math.floor(wallet/(Number(c.price)*1.0025*1.0015)*100)/100;el.shares.value=Math.max(v,0).toFixed(2)}updateEstimate()}
async function submitTrade(){if(tradeBusy||!selectedTicker)return;const sh=shareValue();if(sh<.01){tradeMessage("Enter at least 0.01 shares.","error");return}tradeBusy=true;el.submit.disabled=true;try{const {data,error}=await supabaseClient.rpc("execute_real_stock_trade",{p_ticker:selectedTicker,p_side:tradeSide,p_shares:sh});if(error)throw error;tradeMessage(`${tradeSide==="buy"?"Bought":"Sold"} ${shares(data.shares)} ${data.ticker} shares.${data.market_open?"":" Price was cached while the US market was closed."}`,"success");communityData=null;await loadOverview(true);await loadDetail();updateDetail(company(selectedTicker))}catch(e){tradeMessage(e.message||"The trade could not be completed.","error")}finally{tradeBusy=false;el.submit.disabled=false}}
...
$$("#detail-chart-types button").forEach(b=>{b.classList.toggle("active",b.dataset.chartType===detailChartType);b.onclick=()=>{detailChartType=b.dataset.chartType;localStorage.setItem("real-stock-detail-chart-type",detailChartType);$$("#detail-chart-types button").forEach(x=>x.classList.toggle("active",x===b));drawDetailChart()}});
...
})().catch(e=>{console.error(e);message(e.message||"The real market could not be loaded.","error")});
